...
def evaluate_tensorflow(X_train, X_test, y_train, y_test):
    ...
    if importlib.util.find_spec("tensorflow") is None:
        return None, "TensorFlow is not installed; skipping TensorFlow NN."

    try:
        tf = require_tensorflow()
    except SystemExit as exc:
        return None, f"TensorFlow NN skipped due to TensorFlow runtime issue: {exc}"

    threshold, cv_metrics, tensorflow_version = tune_tensorflow_threshold(X_train, y_train, tf=tf)
