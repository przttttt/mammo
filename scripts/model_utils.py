...
FEATURE_COLUMNS = [f"{name}_{part}" for part in ("mean", "se", "worst") for name in FEATURES]
...
def require_tensorflow():
    try:
        ...
        raise SystemExit(
            "TensorFlow could not be imported cleanly. Reinstall with "
            "requirements/requirements-tensorflow.txt or fix the local TensorFlow runtime. "
...
def load_bundle(path: Path, *, require_model: bool = False, require_scaler: bool = False):
    try:
        bundle = pickle.loads(path.read_bytes())
    except (pickle.UnpicklingError, EOFError, AttributeError, ImportError, IndexError, ValueError) as exc:
        raise SystemExit(
            f"Invalid model artifact at {path}: could not deserialize artifact payload. "
            "Retrain the model artifact."
        ) from exc
    validate_bundle(bundle, path, require_model=require_model, require_scaler=require_scaler)
    return bundle


def _invalid_artifact(path: Path, problem: str):
    raise SystemExit(f"Invalid model artifact at {path}: {problem} Retrain the model artifact.")


def validate_bundle(bundle, path: Path, *, require_model: bool = False, require_scaler: bool = False):
    if not isinstance(bundle, dict):
        _invalid_artifact(path, "expected a dictionary bundle.")

    metadata = bundle.get("metadata")
    if not isinstance(metadata, dict):
        _invalid_artifact(path, "missing metadata block.")
    ...
    if not isinstance(columns, list):
        _invalid_artifact(path, "missing feature column list.")
    if columns != FEATURE_COLUMNS:
        _invalid_artifact(path, "unexpected feature column schema.")

    artifact = metadata.get("artifact")
    if not isinstance(artifact, str) or not artifact:
        _invalid_artifact(path, "missing artifact label.")

    threshold = metadata.get("threshold")
    if isinstance(threshold, bool) or not isinstance(threshold, (int, float)) or not 0.0 <= float(threshold) <= 1.0:
        _invalid_artifact(path, "missing or invalid decision threshold.")

    split = metadata.get("split")
    if not isinstance(split, dict):
        _invalid_artifact(path, "missing split metadata.")

    if "random_state" not in split:
        _invalid_artifact(path, "missing split random_state.")

    if require_model and bundle.get("model") is None:
        _invalid_artifact(path, "missing trained model payload.")

    if require_scaler and bundle.get("scaler") is None:
        _invalid_artifact(path, "missing scaler payload.")
...
def tune_tensorflow_threshold(X_train, y_train, epochs: int = 20, batch_size: int = 32, tf=None):
    if tf is None:
